...
The effects of the interactions described in the previous chapter have many consequences,
both in astrophysics experiments, where the consequences of gravitational interactions are unavoidable, and in particle physics experiments
such as underground DD experiments, fixed target experiments and detectors at particle colliders, where non-gravitational interactions must play a role. 
...
\begin{marginnote}[]
\entry{Run-1}{First period of LHC running (2010-2012) at 7 and 8 TeV center-of-mass energy, where approximately 20 inverse femtobarn of data were collected by ATLAS and CMS.} %this is probably too much information
\entry{Run-2} {Second, ongoing period of LHC running (2015-2018) at 13 TeV center-of-mass energy, planning to collect approximately 100 inverse femtobarns of data.}
\entry{HL-LHC} {High-Luminosity LHC running period, planned to start in 2026 to collect 3000 inverse femtobarn.}
\end{marginnote}
...
%searching for invisible particles at
...
\subsection{Searches for invisible particles with missing transverse momentum - change me}
...
Large \MET causing fake contributions to the highest signal regions in LHC invisible particles searches
...
\subsection{Searches with jets - change me}
...
Fig.~\ref{fig:fakeMET} are rejected with specific quality criteria discussed in the [thing].%Sidebar? Textbox?
...
High-statistics jet+\MET searches need a very precise estimate of the shape of the background events
to be sensitive to signals of feebly interacting particles; 
simulated events are insufficient for this purpose
due to uncertainties on both theory and detector simulation affecting the total cross-sections. 
For this reason, the \MET distribution combines information from both data and the most recent
perturbative calculations~\cite{Lindert:2017olm}.
The shape of this distribution is taken from simulation, but its normalization is derived from data in 
signal-free \textit{control regions} selecting  visible boson+jet processes (W, Z, $\gamma$) 
where the transverse momentum of the visible decay products is subtracted from the total transverse
momentum balance to obtain a proxy of the \MET distribution. 
...
[Consider adding more info about QCD/QED correction and importance thereof].
...
%Model-independent
Having observed no excess over the estimated background, limits are set on the production cross-section of invisible particles, and interpreted in terms of constraints on invisible particle production within simplified models of BSM mediation and Higgs portal. 
...
EFT operators were employed to describe invisible particles reactions at colliders at the Tevatron~\cite{Bai:2010hh,Beltran:2010ww}.
...
%[TODO CD: find completion: maybe Linda's talk at some old invisible particlesWG? AB may have more luck with trilobyte searches]
...
Since the simulation of the entire parameter space for these models by the experiments is computationally intensive, the invisible particles Forum had agreed on a limited set of benchmark parameters to be tested~\cite{Abercrombie:2015wmb}, privileging those that change the LHC kinematics of the search (e.g. give a harder \MET spectrum) rather than those that only change the cross-section of the process and can therefore be reinterpreted from the search results. For example, the kinematics and cross-section of the vector and axial vector mediators is very similar at the LHC, while the DD and ID cross-sections change. 
The parameter values used as benchmarks (e.g. couplings) have been selected considering the sensitivity of early Run-2 searches, precision constraints and general simplicity arguments. As described more in detail in Section~\ref{sub:comparisonVisibleInvisible}, results are given in the \minvisible particles, \mmed  plane fixing the couplings to \gq=0.25 and \ginvisible particles=1.0 for vector and axial vector mediated models, \gq=\ginvisible particles=1.0 for scalar and pseudoscalar models and \ginvisible particlesq=1.0 for colored scalar models. The simplified models employed by the experimental collaborations are known at NLO~\cite{Neubert:2015fka,Haisch:2013ata,Backovic:2015soa}. 
...
have been used to search for invisible particles, due to their relative experimental simplicity
and high rates. Other decay channels such as $ZZ, WW$ and $\tau\tau$ are 
expected to contribute to invisible particles searches as well in the future. 
...
Sec.~\ref{sub:simplifieinvisible particlesodels} with 
...
and on a Z'-2Hinvisible particles model with $tan\beta$=1, \gZPrime=0.8 and \minvisible particles=100 GeV~\footnote{ 
In the case of the Z'-2Hinvisible particles model, CMS and ATLAS set different masses for the 
new Higgs bosons, 
