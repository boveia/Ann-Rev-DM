...
two-body resonances. In the following, we will describe two of the most general examples,
...
If the resonance is wide, as in the case of mediator models with couplings above [X], 
the peak will not be sufficiently distinct from the background. In this case, 
searches exploit the scattering angle of dijet events: 
...
Standard dijet searches lose sensitivity at low masses, where the high QCD
rates force the experiments to randomly discard a large fraction of background and
signal events alike (see sidebar). An example of
such a technique is recording only partial event information for later analysis directly 
from the trigger system (Data Scouting in CMS, Trigger-object Level Analysis in ATLAS, Turbo Stream in LHCb),
to overcome the data storage constraints. 
Dijet resonance searches that use this technique~\cite{CMS-PAS-EXO-16-046,ATLAS:2016xiv}
...
Secondly, the extremely large background rates (up to 10$^X$ events/GeV) grant
a sufficient statistical precision to observe signals of the order of a few thousand events,
but also permille-level detector and SM contamination effects. \textbf{CD: check!} 
An alternative data-taking strategy is to require a high-\pt{} ISR object to trigger
...
\textbf{CD: substitute with PAS}, depending on the ratio between mass and transverse momentum of the resonance
...
\textbf{Trigger prescales} Since the rates of SM physics processes decrease
...
Searches for signals with MET or \pt below these thresholds are penalized
unless novel
%not novel anymore?
data recording techniques, such as only recording partial event information 
needed for the search, are employed.

\textbf{Pile-up in trigger} Simultaneous proton-proton interactions occurring within the detector
...
using information on the event density [cite PUPPI and pile-up suppression in ATLAS]. 
...
ATLAS and CMS from dedicated hardware systems [cite FTK, CMS track trigger]. 
\end{textbox}

%List constraints, why we plot things in the mass-mass plane
...
%Mention other searches: ttbar resonances for pseudoscalar with interference, Higgs-like scalars (CMS boosted) 
...
of the mediator to leptons and jets, due to the much reduced backgrounds. 
%what is the minimum coupling by dilepton searches?
...
such as leptons and quarks~\footnote{For axial vector models, perturbativity issues
arise in the off-shell region so that the model is only valid when 
$\mdm^2\gdm^2/(pi*\mmed^2)<1/2$~\cite{Chala:2015ama,Kahlhoefer:2015bea,Boveia:2016mrp}}.%maybe put this in the reactions chapter
...
%Make sample figure with different scenarios as an example

\begin{figure}[!htpb]
\includegraphics[width=\textwidth]{figures/DMSummary.pdf}
