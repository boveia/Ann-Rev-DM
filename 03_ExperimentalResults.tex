...
Events containing invisible particles can be identified and selected at colliders if initial state radiation (ISR) is present. For $e^+e^-$ colliders, the most frequently radiated object is a photon, while for hadron colliders gluon radiation dominates. Searches for excesses of these events have been termed "Mono-X", where X is the radiated object, although the radiation of a single object is only the leading process in a SM-DM $s-$channel interaction~\cite{Haisch:2013ata}. For this reason, the most recent LHC searches for MET with jets~\cite{Sirunyan:2017jix,Aaboud:2017phn} allow for events containing more than one jet in the final state. 
%up to 3 jets with pT>30 GeV for ATLAS
Events are selected if they contain at least one jet in the central region of the detector ($\eta<2.4$) with \pt $>$ 250 GeV (ATLAS) or \pt $>$ 100 GeV (CMS) and \MET $>$ 250 GeV (ATLAS) or \MET $>$ 200 GeV (CMS). This selection ensures that all events with these characteristics are recorded by the trigger system for further analysis, as described in more detail in Sec.~\ref{sub:trigger}. A lepton veto is used to suppress background from leptonically decaying W bosons. QCD background where large \MET originates from mismeasured jets is rejected by requiring that the $\phi$ direction of the missing transverse momentum vector does not align with the direction of the four-momentum of the jets with the highest \pt (leading jets). Events containing fake \MET due to non-collision background (e.g. cosmic rays, beam-gas interactions, calorimeter problems) is rejected with specific quality criteria discussed in Sec.~\ref{sub:MET}
...
The main background contributions that remain after the event selection are invisible decays of the Z boson into neutrinos (approximately 60\% of the total background) 
%numbers in Livia's talk: https://indico.cern.ch/event/682235/contributions/2817876/attachments/1576792/2490208/DMWG-2017_V2.pdf
and leptonic decays of the W boson where the lepton is not reconstructed (approximately 30\% of the total background), in association with jets. 

%CMS excludes taus, while ATLAS does not. Too much detail? 
...
%Mention that this also works for compressed scenarios

%%Prospects


However, it is not always one single object that gets radiated. 


Searches called "mono-X" have been , as they rely on initial state radiation of a SM object to make the WIMP visible.  
