...
The background estimation technique for the main $\gamma$/V+\MET search backgrounds 
mirrors that of the jet+\MET search, where \MET is emulated by removing visible boson decays from the event energy balance in control regions, and transferring the number of 
measured events to the signal region. 

Another approach is to follow a mono-Higgs event selection and reinterpret the results. 

Jets faking leptons or photons, as well as pions that convert to photons within the detector volume, are reducible backgrounds for these searches. 
