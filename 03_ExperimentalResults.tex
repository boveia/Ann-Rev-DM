...
%- not able to prove non-interacting particles are {\IP}s but
%- discover/study other mediators and other particles in dark sector, not just {\IP}s
...
While collider experiments alone cannot discover DM, they can discover {\IP}s, and this could open up direct study of DM--SM mediators in other channels and of additional particles in a dark sector. %this essentially means we are free from astroparticle uncertainties, more explicit?
...
Below we discuss experimental searches for {\IP}s from three LHC~\cite{LHC2008} experiments, ATLAS, CMS, and LHCb~\cite{ATLAS2008,CMS2008,LHCb2008} and briefly touch on some results from previous colliders. %does ALICE search for invisible particles?
...
%transition????
%We start describing searches for {\IP}s interacting through SM bosons~\ref{sec:results_ZHSearches}, then move to generic searches for signals of {\IP}s with missing transverse momentum and signals of mediators decaying into visible particles~\ref{sec:results_monoXSearches}, outline the searches for complete models with {\IP}s candidates in Section~\ref{sec:results_SUSYSearches} and finally conclude with searches for long-lived particles~\ref{sec:results_LLPSearches}. Throughout this chapter
...
%We then conclude with searches for long-lived particles within models of {\IP}s in
%Section~\ref{sec:results_LLPSearches}. %CD: need to rewrite this sentence, but the idea is: if we hadn't had {\IP}s as a motivation motivation we wouldn't have done this difficult stuff. 
...
%- difference between {\IP}s reactions in SM and new physics models: rates 
...
%\textbf{Decays of the Z boson into {\IP}s} can be constrained using the invisible Z width. It can be measured directly in Z decays in association with a photon emitted as initial state radiation. Events are selected containing a single photon, missing transverse momentum and no other sizable event activity. This selection is also used for identifying events from possible {\IP}s reactions at colliders.
...
%Englert:2011yb,
%% its small width to {\IP}s in the SM is far below current experimental sensitivity, so an observation of invisibly-decaying Higgs would imply new physics. 
...
%We begin this section by describing the LHC searches for missing transverse momentum in association with one or more hadronic jets. The jet+\MET search allows us to illustrate many of the techniques used in invisible particle searches, and it is one of the most powerful to constrain BSM-mediated simplified models of {\IP}s. We then move on to outlining searches using different associated objects, and continue with searches for visible mediators that are the consequences of the {\IP}s production mechanism. Finally, we compare and discuss the sensitivity of invisible {\IP}s and visible mediator searches at the LHC. 
\begin{marginnote}[]
...
%,Aaboud:2016leb,Khachatryan:2016bia}.}
\end{marginnote}

\subsubsection{Searches with jets}

%- searches for {\IP}s (where the interaction is mediated by another particle) all have a common denominator in the large Z + (stuff) to invisible SM background
...
However, these searches provide the most stringent limits on some models where the boson in question is directly involved in the dark interaction~\cite{Berlin:2014cfa}.% excluding EFT scales between 150 and 750 GeV for \mdm=100 GeV, assuming the maximal coupling value allowed by perturbativity.
%Petrov:2013nia
...
%The masses of the neutral CP-even scalar (H0) and the charged scalars (H�) from Z?-2H{\IP}s model are set to 300 GeV. The {\IP}s mass m? is set to 100 GeV 
%CMS: 
%Two-Higgs-doublet-Z' signals with a pseudoscalar mass of 300 GeV are excluded at 95\% CL for Z' masses below 900 GeV
%Baryonic Z' models with a {\IP}s mass of 1 GeV are excluded at 95% CL for Z' masses below 800 GeV
...
%Generic searches employing one single additional type of object produced in association with \MET are powerful tools to probe simple models of {\IP}s. 
...
%The increased LHC dataset will allow these searches to be sensitive for other {\IP}s masses. 
...
%model discussed in~\cite{Agrawal:2014una} are excluded up to 1.1 TeV for {\IP}s masses of 35 GeV. 
...
%scalar and pseudoscalar mediators decaying into {\IP}s (including the case of colored scalars), 
...
%These searches place constraints on models of {\IP}s (resonant, non-resonant). 
...
%so far target prompt production of WIMP {\IP}s or associated mediator particles at colliders. 
...
%- coming from {\IP}s: why would we do a search for visible particles and no {\IP}s?
...
%So far, we have discussed searches for {\IP}s as a tool to probe the interaction between the SM and the dark sector. 
...
%Constraints on Z? models from LHC dijet searches and implications for {\IP}s - https://arxiv.org/pdf/1605.07940.pdf -> ok
...
%% assumes that the new particle can decay only to quarks and {\IP}s particles, with \gdm=1.0 and \mdm=1 GeV.}, the fitted background estimation may be biased by the presence of signal.
...
Alternatively, one can look at the subset of dijet events where a high-\pt{} ISR object happens to trigger~\cite{ATLAS:2016bvn,Sirunyan:2017nvi}. %jet and photon+\MET searches above.
...
%lepton couplings feature in a variety of models that can embed the simplified models of {\IP}s used as benchmark for LHC searches. %this sentence repeats the one in Sec.2. 
...
%up to the point where the resonance
%Limits are read off at discrete points from published plots, these points chosen to mitigate the effects of interpolation by assuming that limits were set at signal points generated with round numbers for mZ?. These points are then drawn between these points, truncated where two lines from a set cross.
...
Fig.\cite{fig:couplingmass} illustrates constraints from the dijet resonance searches above, on the quark coupling of the mediator in an axial-vector simplified model, as a function of the mediator mass, for a model that assumes no tree-level couplings to leptons and no possible decay to DM.
...
%The CMS analysis also scans the coupling-mass plane by fixing the ratio between \m{\IP}s and \mmed to ensure perturbativity 
...
%Mention LianTao's paper where baryonic / 2H{\IP}s monoHiggs is also constrained. 
...
%; the region constrained by from \MET+jet searches extends to lower {\IP}s and mediator masses with respect to the case of the model with \gq=0.25 due to the reduced production rate. 
...
%and as such they alone can offer little information on whether a discovery would imply in terms of {\IP}s mediators.
