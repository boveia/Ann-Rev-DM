...
but that was not gauge invariant~\cite{Bell:2015sza}.}. 
...
Z+\MET searches are also sensitive to the radiation of a boson 
...
The LHC provides collision events every 25 ns. This amount
of data cannot be recorded in its entirety, and it
is not all interesting for the experiments' physics program: the
trigger system decides whether an event is selected for further
analysis. Its first level is realized in hardware and only uses
partial detector information for fast decisions in the order of
$\mu s$, while its second level is software-based and uses more
refined algorithms and information. 
