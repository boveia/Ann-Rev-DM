...
the searches for dijet and dilepton resonances, and the implication of their results
for models including a Z'-like mediator. 

%Describe dijet search

%Where dijets lose sensitivity: low-mass and angular 
...
arise in the off-shell region so that the model is only valid when $\mdm^2\gdm^2/(pi*\mmed^2)<1/2$~\cite{Chala:2015ama,Kahlhoefer:2015bea,Boveia:2016mrp}}.%maybe put this in the reactions chapter
...
unless novel
%not novel anymore?
data recording techniques are employed. An example of
such a technique is recording only partial event information for later analysis
(Data Scouting in CMS, Trigger-object Level Analysis in ATLAS, Turbo Stream in LHCb),
to overcome the data storage constraints. Dijet resonance searches using this technique
are used to reach lower DM mediator masses than the standard dijet searches
where full events are recorded. 
