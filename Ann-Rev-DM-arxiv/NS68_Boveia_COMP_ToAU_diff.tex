...
%DIF DEL NS68_Boveia_COMP_ToAU.TEX         Fri Jul 20 10:00:03 2018
%DIF ADD NS68_Boveia_COMP_ToAU_ICHEP.tex   Fri Jul 20 10:00:03 2018
...
\begin{figure}[!htpb]
%DIF < \includegraphics[width=\textwidth]{figures/feynman_0}
...
$h_B$~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Berlin:2014cfa}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Carpenter:2013xra}}%DIFAUXCMD
...
\begin{figure}[!htpb]
%DIF < \includegraphics[width=\textwidth]{figures/feynman_1}
...
\begin{figure}[!htpb]
%DIF < \includegraphics[width=\textwidth]{figures/feynman_2}
...
\begin{figure}[!htpb]
%DIF < \includegraphics[width=0.5\textwidth]{figures/FakeMET}
...
versus 200 GeV for the jet+\MET \DIFdelbegin \DIFdel{search~\mbox{%DIFAUXCMD
\cite{Sirunyan:2017jix}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{searches (e.g.~\mbox{%DIFAUXCMD
\cite{Sirunyan:2017jix}}%DIFAUXCMD
)}\DIFaddend .
...
Photon+\MET searches~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Aaboud:2017dor,CMS-PAS-EXO-16-014} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Aaboud:2017dor,CMS-PAS-EXO-16-053} }%DIFAUXCMD
}\DIFaddend has lower backgrounds
...
in the 
$H \rightarrow \gamma\gamma$~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{CMS-PAS-EXO-16-054,Aaboud:2017uak} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Sirunyan:2018fpy,Aaboud:2017uak}}%DIFAUXCMD
, 
$H \rightarrow \tau\tau$~\mbox{%DIFAUXCMD
\cite{Sirunyan:2018fpy}}%DIFAUXCMD
, 
}\DIFaddend and
$H \rightarrow b\bar{b}$~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Aaboud:2017yqz} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Aaboud:2017yqz,CMS-PAS-EXO-16-050} }%DIFAUXCMD
}\DIFaddend channels have been
performed. Searches in the \DIFdelbegin \DIFdel{$ZZ,~WW$, and $\tau\tau$ }\DIFdelend \DIFaddbegin \DIFadd{$ZZ and~WW$ }\DIFaddend channels are
expected to contribute as well, once substantially more data have been
collected.

The \MET+$H \rightarrow \gamma\gamma$
searches~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{CMS-PAS-EXO-16-054,Aaboud:2017uak} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Sirunyan:2018fpy,Aaboud:2017uak} }%DIFAUXCMD
}\DIFaddend benefit from
...
as 50 GeV~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{CMS-PAS-EXO-16-054}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Sirunyan:2018fpy}}%DIFAUXCMD
}\DIFaddend . The diphoton-invariant mass
is fitted in different signal categories, each optimized for
different types of signal models. 
The \DIFdelbegin \DIFdel{search }\DIFdelend \DIFaddbegin \DIFadd{$H \rightarrow \tau\tau$ search also benefits from lower SM backgrounds 
than other }\MET\DIFadd{+$H$ searches, as it selects events based on the presence 
of $\tau$ leptons rather than on the presence of large }\MET\DIFadd{. This search 
is sensitive to anomalous }\MET \DIFadd{signals above 105 GeV. 
The search }\DIFaddend for a Higgs boson decaying to
two bottom quarks~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Aaboud:2017yqz} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Aaboud:2017yqz,CMS-PAS-EXO-16-050} }%DIFAUXCMD
}\DIFaddend requires \MET$>$150 \DIFdelbegin \DIFdel{GeV.
All backgrounds except for the QCD background are estimated using
Monte Carlo simulation and constrained in dedicated control regions. This search
also employs }\DIFdelend \DIFaddbegin \DIFadd{or 200 GeV.
These searches employ }\DIFaddend jet substructure techniques \DIFdelbegin \DIFdel{for }%DIFDELCMD < \MET%%%
\DIFdel{$>$500 GeV }\DIFdelend to
select boosted Higgs decays amid a background of QCD processes.
The \DIFdelbegin \DIFdel{main systematic uncertainty for the lower-}%DIFDELCMD < \MET %%%
\DIFdel{signal region
is the modeling of the }\textit{\DIFdel{V}}%DIFAUXCMD
\DIFdel{+jets background, while a higher-}%DIFDELCMD < \MET
%DIFDELCMD < %%%
\DIFdel{signal region is still statistically limited with the current
data set}\DIFdelend \DIFaddbegin \DIFadd{majority of the backgrounds are estimated using
data-driven techniques and Monte Carlo simulation and constrained in dedicated control regions}\DIFaddend . 

In the absence of a signal, limits are placed on the baryonic Higgs benchmark
model (outlined in Section~\ref{sub:simplifiedModels} and shown in Figure~\ref{fig:feynman_1}\textit{\DIFdelbegin \DIFdel{c}\DIFdelend \DIFaddbegin \DIFadd{a}\DIFaddend })
with \gq = 1\DIFaddbegin \DIFadd{/3 (ATLAS) or 0.25 (CMS)}\DIFaddend , \gdm = 1, and \ghZprimeZprime/$m_{Z}$ = 1, \DIFdelbegin \DIFdel{and }\DIFdelend on a
\Zprime--2HDM (Figure~\ref{fig:feynman_1}\textit{\DIFdelbegin \DIFdel{d}\DIFdelend \DIFaddbegin \DIFadd{b}\DIFaddend }).
...
\DIFaddbegin \DIFadd{Both }\DIFaddend Higgs+\MET and \textit{Z}+\MET searches are also sensitive to extended
scalar sectors such as two Higgs doublets with a scalar or
pseudoscalar mediator~\cite{Bauer:2017ota,Goncalves:2016iyg,Bell:2016ekl}\DIFaddbegin \DIFadd{;
the search in Reference }{\DIFadd{CMS-PAS-EXO-16-050}} \DIFadd{sets limits on the 2HDM + pseudoscalar model of~\mbox{%DIFAUXCMD
\cite{Bauer:2017ota}
}%DIFAUXCMD
shown in Figure ~\ref{fig:feynman_1}}\textit{\DIFadd{c}}\DIFaddend .
...
}\DIFdelend (e.g.,~\citen{Aaboud:2017bac,Sirunyan:2017yse}); by contrast, other processes are
...
\begin{figure}[!htpb]
%DIF < \includegraphics[width=\textwidth]{figures/SUSY_electroweak.pdf}
\DIFaddbeginFL \includegraphics[width=\textwidth]{figs_standalone/CMS-SUS-17-004_Figure_010-a.pdf}
\includegraphics[width=\textwidth]{figs_standalone/ATLAS_SUSY_EWSummary_higgsino.pdf}
\DIFaddendFL \caption{Mass reach of (\textit{a})\ CMS and (\textit{b})\ ATLAS searches for a selection of
results targeting electroweak supersymmetry production, available as of
\DIFdelbeginFL \DIFdelFL{December 2017.
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{July 2018.
}\DIFaddendFL Panel \textit{a} adapted from Reference \DIFdelbeginFL \DIFdelFL{\mbox{%DIFAUXCMD
\citen{CMSSUSYSummary}}%DIFAUXCMD
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\citen{Sirunyan:2018ubx}}%DIFAUXCMD
}\DIFaddendFL . 
Panel \textit{b} adapted from Reference~\DIFdelbeginFL \DIFdelFL{\mbox{%DIFAUXCMD
\citen{ATLASSUSYSummary}}%DIFAUXCMD
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\citen{ATL-PHYS-PUB-2017-019}}%DIFAUXCMD
}\DIFaddendFL .\label{fig:SUSYSummary_ew}}
\end{figure}
...
\begin{figure}[!htpb]
%DIF < \includegraphics[width=\textwidth]{figures/CouplingMassPlot.pdf}
\DIFaddbeginFL \includegraphics[width=\textwidth]{figs_standalone/CouplingMassPlot.pdf}
\DIFaddendFL \caption{Summary of constraints from searches for \DIFaddbeginFL \DIFaddFL{narrow, }\DIFaddendFL light dijet
resonances from ATLAS and CMS \DIFaddbeginFL \DIFaddFL{available as of
July 2018}\DIFaddendFL , where discrete points are taken
...
\DIFdelbeginFL \DIFdelFL{15}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{10-15}\DIFaddendFL \% (roughly \gq $>$ 0.5). Abbreviation:\ DM, dark matter.
Pre-LHC constraints are \DIFaddbeginFL \DIFaddFL{extracted }\DIFaddendFL from Reference \citen{Dobrescu:2013coa}\DIFaddbeginFL \DIFaddFL{,
while LHC constraints are taken from References \mbox{%DIFAUXCMD
\cite{Sirunyan:2017nvi,Aaboud:2018zba,
ATLAS:2016bvn,Sirunyan:2017nvi,Sirunyan:2018xlo,Aaboud:2018fzt,Aaboud:2017yvp}}%DIFAUXCMD
}\DIFaddendFL .}
...
\begin{figure}[!htpb]
%DIF < \includegraphics[width=\textwidth]{figures/SummaryPlotsMassMass.pdf}
\DIFaddbeginFL \includegraphics[width=0.7\textwidth]{figs_standalone/ATLAS_DarkMatter_Summary_Vector.pdf}\\
\includegraphics[width=0.7\textwidth]{figs_standalone/ATLAS_DarkMatter_Summary_Vector_ModifiedCoupling.pdf}
\DIFaddendFL \caption{Regions in DM mass--\Zprime mediator mass
excluded at 95\% CL by a selection of ATLAS \DIFdelbeginFL \DIFdelFL{dark matter }\DIFdelendFL searches 
\DIFaddbeginFL \DIFaddFL{(from References \mbox{%DIFAUXCMD
\cite{ATLAS:2016bvn,Aaboud:2018fzt,Aaboud:2017yvp,
Aaboud:2017phn,ATLAS-CONF-2018-005,Aaboud:2017bja,Aaboud:2017dor,
Aaboud:2017buh}}%DIFAUXCMD
) 
available as of July 2018, }\DIFaddendFL for two coupling scenarios. Dashed curves labeled ``thermal
...
DM. 
\DIFaddbeginFL \DIFaddFL{In panel (a), the couplings of the mediator particle to each generation
of quarks (}\gq\DIFaddFL{) are set to 0.25, the couplings to leptons (}\gl\DIFaddFL{)
are set to zero and the coupling to DM is set to unity. 
In panel (b), }\gq \DIFaddFL{is set to 0.1, }\gl \DIFaddFL{is set to 0.01
and the coupling to DM }\gdm \DIFaddFL{is set to unity and marked as $g_{DM}$ in this plot.
}\DIFaddendFL Abbreviations:\ DM, dark matter; ISR, initial-state radiation; 
...
\begin{figure}[!htpb]
%DIF < \includegraphics[width=0.9\textwidth]{figures/SI_CMSDD_Summary}
\DIFaddbeginFL \includegraphics[width=0.9\textwidth]{figs_standalone/SI_CMSDD_Summary}
\DIFaddendFL \caption{The 90\%-CL constraints from the CMS experiment 
\DIFaddbeginFL \DIFaddFL{from References \mbox{%DIFAUXCMD
\cite{Sirunyan:2017nvi,Sirunyan:2018xlo,Sirunyan:2017jix,CMS-PAS-EXO-16-053,Sirunyan:2017qfc}
}%DIFAUXCMD
}\DIFaddendFL in the \mdm-spin-independent DM--nucleon plane for a vector mediator,
Dirac DM, and benchmark couplings \gq = 0.25 and \gdm = 1.0 \DIFaddbeginFL \DIFaddFL{(marked as $g_{DM}$ in this plot) }\DIFaddendFL chosen as an example of what
early LHC searches would be sensitive to, compared with direct detection
experiments \DIFaddbeginFL \DIFaddFL{from References \mbox{%DIFAUXCMD
\cite{Angloher:2015ewa,Agnese:2015nto,Cui:2017nnn,Akerib:2016vxi,Aprile:2018dbl}}%DIFAUXCMD
}\DIFaddendFL . 
\DIFaddbeginFL \DIFaddFL{It is important to note that this comparison is only valid for this particular
combination of model and parameter choices. 
}\DIFaddendFL Abbreviation:\ DM, dark matter. Adapted from Reference~\citen{CMSSummary}.} \label{fig:SICMS}
\end{figure}
...
\DIFaddbegin \bibitem{Carpenter:2013xra} 
\DIFadd{Carpenter L, et~al. }\textit{\DIFadd{Phys.\ Rev.\ D}} \DIFadd{89:075017 (2014)
}

\DIFaddend \bibitem{Chala:2015ama}
...
\DIFdelbegin %DIFDELCMD < \bibitem{CMS-PAS-EXO-16-014}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibitem{CMS-PAS-EXO-16-053}
\DIFaddend CMS Collab. \textit{Search for \DIFdelbegin \DIFdel{dark matter and large extra dimensions }\DIFdelend \DIFaddbegin \DIFadd{new physics }\DIFaddend in \DIFdelbegin \DIFdel{the}\DIFdelend \DIFaddbegin \DIFadd{final states with a single photon plus missing transverse momentum in proton-proton collisions at}\DIFaddend } \DIFdelbegin \DIFdel{$\gamma$ + }%DIFDELCMD < \MET %%%
\textit{\DIFdel{final state in pp Collisions at}} %DIFAUXCMD
\DIFdelend $\sqrt{s}$=\textit{13 TeV} \DIFdelbegin \DIFdel{. Report CMS-PAS-EXO-16-014}\DIFdelend \DIFaddbegin \DIFadd{using 2016 data. Report CMS-PAS-EXO-16-053}\DIFaddend , CERN, Geneva (2016)
...
\DIFdelbegin %DIFDELCMD < \bibitem{CMS-PAS-EXO-16-054}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibitem{Sirunyan:2018fpy}
\DIFaddend CMS Collab. \DIFdelbegin \textit{\DIFdel{Search for dark matter produced in association with a Higgs boson decaying to two photons.}}
%DIFAUXCMD
\DIFdel{Report CMS-PAS-EXO-16-054, CERN, Geneva (2017}\DIFdelend \DIFaddbegin \DIFadd{arXiv:1806.04771 }[\DIFadd{hep-ex}] \DIFadd{(2018}\DIFaddend )
...
\DIFaddbegin \bibitem{CMS-PAS-EXO-16-050}
\DIFadd{CMS Collab.
}\textit{\DIFadd{Search for associated production of dark matter with a Higgs boson that decays to a pair of bottom quarks}}
\DIFadd{Report CMS-PAS-EXO-16-050, CERN, Geneva (2018)
}

\DIFaddend \bibitem{Aaboud:2017rzf}
{ATLAS Collab.}  \textit{Eur.\ Phys.\ J.\ C} {78}:18 (2018)

\bibitem{Aaboud:2017aeu}
{ATLAS Collab.} \DIFdelbegin \DIFdel{arXiv:1711.11520 }%DIFDELCMD < [%%%
\DIFdel{hep-ex}%DIFDELCMD < ] %%%
\DIFdel{(2017}\DIFdelend \DIFaddbegin \textit{\DIFadd{J. High Energy Phys.}} \DIFadd{06:108 (2018}\DIFaddend )
...
{CMS Collab.} \DIFdelbegin \DIFdel{arXiv:1801.08427 }%DIFDELCMD < [%%%
\DIFdel{hep-ex}%DIFDELCMD < ] %%%
\DIFdelend \DIFaddbegin \textit{\DIFadd{J. High Energy Phys.}} \DIFadd{06:027 }\DIFaddend (2018)
...
\DIFdelbegin %DIFDELCMD < \bibitem{CMSSUSYSummary}
%DIFDELCMD < {%%%
\DIFdel{CMS Collab}%DIFDELCMD < }%%%
\DIFdel{. }\textit{\DIFdel{Supersymmetry public results}}%DIFAUXCMD
\DIFdel{. CERN, Geneva.
}%DIFDELCMD < \url{https://twiki.cern.ch/twiki/bin/view/CMSPublic/PhysicsResultsSUS} %%%
\DIFdel{(2018)
}%DIFDELCMD < 

%DIFDELCMD < \bibitem{ATLASSUSYSummary}
%DIFDELCMD < {%%%
\DIFdel{ATLAS Collab}%DIFDELCMD < }%%%
\DIFdel{. }\textit{\DIFdel{Supersymmetry public results}}%DIFAUXCMD
\DIFdel{. CERN, Geneva. }%DIFDELCMD < \url{https://twiki.cern.ch/twiki/bin/view/AtlasPublic/SupersymmetryPublicResults} %%%
\DIFdel{(2018)
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \bibitem{Sirunyan:2018ubx}
{CMS Collab.} \DIFdelbegin \DIFdel{arXiv:1801.03957 }%DIFDELCMD < [%%%
\DIFdel{hep-ex}%DIFDELCMD < ] %%%
\DIFdelend \DIFaddbegin \textit{\DIFadd{J. High Energy Phys.}} \DIFadd{160:1803 }\DIFaddend (2018)
...
{ATLAS Collab}. \DIFdelbegin \DIFdel{arXiv:1712.08119 }%DIFDELCMD < [%%%
\DIFdel{hep-ex}%DIFDELCMD < ] %%%
\DIFdel{(2017}\DIFdelend \DIFaddbegin \textit{\DIFadd{Phys. Rev.}} \textit{\DIFadd{D}} \DIFadd{97:052010 (2018}\DIFaddend )
...
{CMS Collab}. \DIFdelbegin \DIFdel{arXiv:1802.02110 }%DIFDELCMD < [%%%
\DIFdel{hep-ex}%DIFDELCMD < ] %%%
\DIFdelend \DIFaddbegin \textit{\DIFadd{J. High Energy Phys.}} \DIFadd{05:025 }\DIFaddend (2018)
...
Mastercode Collab. \DIFdelbegin \DIFdel{arXiv:1710.11091 }%DIFDELCMD < [%%%
\DIFdel{hep-ph}%DIFDELCMD < ] %%%
\DIFdel{(2017}\DIFdelend \DIFaddbegin \textit{\DIFadd{Eur. Phys. J.}} \textit{\DIFadd{C}} \DIFadd{78:256 (2018}\DIFaddend )

\bibitem{Aaboud:2017mpt}
{ATLAS Collab}. \DIFdelbegin \DIFdel{arXiv:1712.02118 }%DIFDELCMD < [%%%
\DIFdel{hep-ex}%DIFDELCMD < ] %%%
\DIFdel{(2017}\DIFdelend \DIFaddbegin \textit{\DIFadd{J. High Energy Phys.}} \DIFadd{06:022 (2018}\DIFaddend )
...
{LHCb Collab}.  \DIFdelbegin \DIFdel{arXiv:1710.02867 }%DIFDELCMD < [%%%
\DIFdel{hep-ex}%DIFDELCMD < ] %%%
\DIFdel{(2017}\DIFdelend \DIFaddbegin \textit{\DIFadd{Phys. Rev. Lett.}} \DIFadd{120:061801 (2018}\DIFaddend )
...
{CMS Collab}. \DIFdelbegin \DIFdel{arXiv:1710.00159 }%DIFDELCMD < [%%%
\DIFdel{hep-ex}%DIFDELCMD < ] %%%
...
physics group}. CERN, Geneva. \url{https://atlas.web.cern.ch/Atlas/GROUPS/PHYSICS/CombinedSummaryPlots/EXOTICS/index.html#ATLAS_DarkMatter_Summary} (\DIFdelbegin \DIFdel{2017}\DIFdelend \DIFaddbegin \DIFadd{2018}\DIFaddend )
...
\DIFaddbegin \bibitem{Angloher:2015ewa} 
\DIFadd{CRESST Collab. }\textit{\DIFadd{Eur. Phys. J.}} \textit{\DIFadd{C}} \DIFadd{76:25 (2016)
}

\bibitem{Agnese:2015nto} 
\DIFadd{CDMS Collab. }\textit{\DIFadd{Phys. Rev. Lett.}} \DIFadd{116:071301 (2016)
}

\bibitem{Cui:2017nnn} 
\DIFadd{PandaX-II Collab. }\textit{\DIFadd{Phys. Rev. Lett.}} \DIFadd{119:181302 (2017)
}

\bibitem{Akerib:2016vxi} 
\DIFadd{LUX Collab. }\textit{\DIFadd{Phys. Rev. Lett.}} \DIFadd{118:021303 (2017)
}

\bibitem{Aprile:2018dbl} 
\DIFadd{XENON Collab. arXiv:1805.12562 }[\DIFadd{hep-ex}] \DIFadd{(2018)
}

\DIFaddend \bibitem{CMSSummary}
CMS Collab. \textit{Dark \MakeLowercase{Matter Summary Plots}\DIFdelbegin \DIFdel{from CMS for LHCP and EPS 2017}\DIFdelend }. CERN, Geneva. \DIFdelbegin %DIFDELCMD < \url{https://twiki.cern.ch/twiki/pub/CMSPublic/PhysicsResultsEXO/DM-summary-plots-Jul17.pdf} %%%
\DIFdel{(2017}\DIFdelend \DIFaddbegin \url{https://twiki.cern.ch/twiki/bin/view/CMSPublic/SummaryPlotsEXO13TeV#Dark_Matter_Summary_plots} \DIFadd{(2018}\DIFaddend )

\bibitem{Busoni:2014gta}
Busoni G, et~al. \textit{J. Cosmol. Astropart. Phys.} 1503:022 (2015)

\bibitem{Catena:2017xqq}
Catena R, Conrad J, Krauss MB. \DIFdelbegin \DIFdel{arXiv:1712.07969 }%DIFDELCMD < [%%%
\DIFdel{hep-ph}%DIFDELCMD < ] %%%
...
\DIFaddend \bibitem{Blumenschein:2018gtm}
