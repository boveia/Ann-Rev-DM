...
In the above, we've sketched some of the models and signatures that are currently being sought in collider searches. In the next part, we will review the status of experimental searches for invisible particles, focusing on those searches that target the models discussed above. Nevertheless, we have left many models uncovered. From a certain point of view, any model including a new particle interacting [feebly] is a theory of DM. Drawing the the connections between these models, 
%why between themselves?
their predicted interactions between SM and 'dark' sectors, and the observed DM abundance is challenging and ongoing [research topic/ / thing ] but this connection is the key difference between models focused on explaining the observed properties of DM, especially its present abundance, and models developed for other purposes such as explaining the hierarchy problem. 

One can think of DM theories as classes of models postulaitng new "dark sectors" that only have feeble connections with the SM or are secluded from it. This dark sector can be arbitrarily complex, as long as the combination of its particles and interactions satisfies current cosmological observations~\cite{Strassler:2006im}. The models listed above are simple examples of such dark sectors, where the mediator particles (e.g. dark bosons) provide the connection with the SM. Other ideas within this framework that we consider worth mentioning are asymmetric DM models where dark sector particles and antiparticles are not produced in equal amount, in the same fashion as matter and antimatter for SM baryons~\cite{Zurek:2013wia}, and models of "neutral naturalness" realizing a mirror copy of the SM without any low-mass equivalent of the SUSY colored partners that evade the experimental constraints described in the next chapter~\cite{Craig:2014aea}. These models provide non-standard signatures that can be tested at colliders beyond the LHC. 
