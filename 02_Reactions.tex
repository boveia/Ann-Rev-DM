...
In some situations, such as when the mediator is heavy compared to the collision energy, the DM--SM interaction appears to be a contact interaction, with all observables completely determine by one rate parameter, the contact interacion scale, that controls the production rate, and a Lorentz structure choice, which has a modest effect on the transverse momentum distributions of the invisible particles.
...
If, instead, the interaction physics (e.g., the mediator mass) is within reach of the typical momentum transfer in the collision, one can profit from specifying further details of the SM--DM interactions~\cite{Fox:2011pm}.
%~\cite{Buchmueller:2013dya,DeSimone:2016fbz,Berlin:2014cfa}, 
Without those details, one can still use the EFT language to provide results for later reinterpretation once a completion is known~\cite{Racco:2015dxa,Busoni:2013lha}. 
...
A mechanism for generating a relic density that is consistent with very weak SM interactions such as those of dark photons is the freeze-in scenario, see e.g. Refs.~\cite{Co:2015pka,Bernal:2017kxu}. 
