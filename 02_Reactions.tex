...
%Caveat??
In the following sections will describe models from the perspective of experimental collider physicists, focusing on a selection of models that provide distinct and testable LHC signatures, without the ambition of theoretical completeness. For other perspectives on the models used for early LHC searches for DM, see~\ref{Kahlhoefer:2017dnp,Abercrombie:2015wmb,Arcadi:2017kky,Ellis:2010kf}. %Cite SUSY review by Feng?
...
As stated in the introduction to this chapter, the LHC does not directly detect DM. Rather, it uses visible objects to signal the presence of non-interacting, long-lived particles that escape detection. If the particle does not decay, then it is a good DM candidate. In a dark sector, one can postulate the existence of other particles with lifetimes not long enough 
to be cosmologically stable, but still escape conventional detection as they e.g. decay half-way through the detector. 

Mechanisms for a particle to acquire a lifetime are , as in the case of coannihilation,

as stated before, the LHC does not detect DM per se, detects invisible particles that don't interact with detector and live long enough to escape before decay
